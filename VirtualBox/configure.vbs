...
sub CheckForVisualCPP(strOptVC, strOptVCCommon, blnOptVCExpressEdition)
   ...
   if   (InStr(1, g_strShellOutput, "16.00.40219.01") <= 0) _
   ...
   '
   if InStr(1, g_strShellOutput, "16.00.40219.01") > 0 then
...
sub CheckForSsl(strOptSsl, bln32Bit)
   ...
   if bln32Bit = True then
      CfgPrint "SDK_VBOX_OPENSSL-x86_INCS := " & strPathSsl & "/x86/include"
      CfgPrint "SDK_VBOX_OPENSSL-x86_LIBS := " & strPathSsl & "/x86/lib/libcrypto.lib" & " " & strPathSsl & "/x86/lib/libssl.lib"
      CfgPrint "SDK_VBOX_BLD_OPENSSL-x86_LIBS := " & strPathSsl & "/x86/lib/libcrypto.lib" & " " & strPathSsl & "/x86/lib/libssl.lib"
   else
      CfgPrint "SDK_VBOX_OPENSSL_INCS := " & strPathSsl & "/x64/include"
      CfgPrint "SDK_VBOX_OPENSSL_LIBS := " & strPathSsl & "/x64/lib/libcrypto.lib" & " " & strPathSsl & "/x64/lib/libssl.lib"
      CfgPrint "SDK_VBOX_BLD_OPENSSL_LIBS := " & strPathSsl & "/x64/lib/libcrypto.lib" & " " & strPathSsl & "/x64/lib/libssl.lib"
   end if
...
function CheckForSslSub(strPathSsl)
   ...
   if   LogFileExists(strPathSsl, "/x86/include/openssl/md5.h") then
         CheckForSslSub = True
      end if
end function
...
sub CheckForCurl(strOptCurl, bln32Bit)
   ...
   if bln32Bit = True then
      CfgPrint "SDK_VBOX_LIBCURL-x86_INCS := " & strPathCurl & "/x86/include"
      CfgPrint "SDK_VBOX_LIBCURL-x86_LIBS.x86 := " & strPathCurl & "/x86/lib/libcurl_a.lib"
   else
      CfgPrint "SDK_VBOX_LIBCURL_INCS := " & strPathCurl & "/x64/include"
      CfgPrint "SDK_VBOX_LIBCURL_LIBS := " & strPathCurl & "/x64/lib/libcurl_a.lib"
   end if
...
function CheckForCurlSub(strPathCurl)
   ...
   if   LogFileExists(strPathCurl, "/x86/include/curl/curl.h") then
