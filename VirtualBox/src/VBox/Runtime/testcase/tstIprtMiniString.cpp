...
#define CHECK_EQUAL_I(iRes, iExpect) \
    ...
    strTest = "abcdef??bcdef";
            // 13 codepoints, but 15 bytes (excluding null terminator);
            // "?" and "?" consume two bytes each
    CHECK_EQUAL(strTest.substr(0),   strTest.c_str());
    CHECK_EQUAL(strTest.substrCP(0), strTest.c_str());

    CHECK_EQUAL(strTest.substr(2),   "cdef??bcdef");
    CHECK_EQUAL(strTest.substrCP(2), "cdef??bcdef");

    CHECK_EQUAL(strTest.substr(2, 2),   "cd");
    CHECK_EQUAL(strTest.substrCP(2, 2), "cd");

    CHECK_EQUAL(strTest.substr(6),   "??bcdef");
    CHECK_EQUAL(strTest.substrCP(6), "??bcdef");

    CHECK_EQUAL(strTest.substr(6, 2),   "?");           // UTF-8 "?" consumes two bytes
    CHECK_EQUAL(strTest.substrCP(6, 1), "?");

    CHECK_EQUAL(strTest.substr(8),   "?bcdef");         // UTF-8 "?" consumes two bytes
    CHECK_EQUAL(strTest.substrCP(7), "?bcdef");

    CHECK_EQUAL(strTest.substr(8, 3),   "?b");          // UTF-8 "?" consumes two bytes
    CHECK_EQUAL(strTest.substrCP(7, 2), "?b");
    ...
    size_t pos = strTest.find("?");
    CHECK_EQUAL(strTest.substr(pos), "??bcdef");
